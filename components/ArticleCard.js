...
export default function ArticleCard({ article, categories, onClick }) {
  ...
  return (
    <div className="article-card" onClick={() => onClick(article)}>
      {/* Category badge */}
      <div className="card-cat" style={{
        background: cat.bg  || 'rgba(255,255,255,0.05)',
        color:      cat.color || 'var(--muted)',
      }}>
        <span className="card-dot" style={{background: cat.color || 'var(--muted)'}} />
        {article.category}
      </div>
      ...
      <div className="card-footer">
        <div className="card-author-wrap">
          <div className="card-avatar" style={{background: avatarColor}}>
            {article.initials || (article.author?.slice(0,2).toUpperCase()) || '?'}
          </div>
          <span>{article.author}</span>
        </div>
        <span>{relDate(article.created_at)}</span>
      </div>
    </div>
  )
}
